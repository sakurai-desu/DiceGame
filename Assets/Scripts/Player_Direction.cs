...
public class Player_Direction : MonoBehaviour {
    ...
    public void Start_Direction(GameObject player) {
        ...
        Player_Direction_Change(player,g_start_Para);
    }

    public void Player_Direction_Change(GameObject player,int para) {
        g_player_Obj = player;
        switch (para) {
            case g_ver_plus_Para:
                g_player_Obj.transform.Rotate(0, 270, 0);
                break;
            case g_ver_minus_Para:
                g_player_Obj.transform.Rotate(0, 90, 0);
                break;
            case g_side_plus_Para:
                g_player_Obj.transform.Rotate(0, 180, 0);
                break;
            case g_side_minus_Para:
                g_player_Obj.transform.Rotate(0, 0, 0);
                break;
        }
    }
}
